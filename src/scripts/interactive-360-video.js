/**
 * H5P Interactive 360 Video
 * 
 * This file is a derivative work based on H5P Interactive Video.
 * 
 * Original work:
 * Copyright (c) 2012-2014 Joubel AS
 * Copyright (c) 2017 H5P
 * 
 * Modifications and extensions for Interactive 360 Video:
...
// Timeout and delay constants (milliseconds)
const INIT_360_RETRY_DELAY_MS = 200;
const INIT_360_INITIAL_DELAY_MS = 100;
const INIT_360_FALLBACK_DELAY_MS = 500;
const INIT_360_MAX_RETRIES = 10;
const PREVENT_SKIPPING_WARNING_DELAY_MS = 500;
const BUFFER_UPDATE_INTERVAL_MS = 500;

// Three.js configuration constants
const THREE_CAMERA_FOV = 75;
const THREE_CAMERA_NEAR = 0.1;
const THREE_CAMERA_FAR = 2000;
const THREE_SPHERE_RADIUS = 500;
const THREE_SPHERE_WIDTH_SEGMENTS = 60;
const THREE_SPHERE_HEIGHT_SEGMENTS = 40;
const THREE_NDC_Z_COORDINATE = 0.5;
...
const NDC_MIN = -1;
const NDC_MAX = 1;
const NDC_CONVERSION_FACTOR = 2;

/**
 * @typedef {Object} Interactive360VideoParameters
 * @property {Object} interactive360Video View parameters
...
/**
 * Initialize a new interactive 360 video.
 *
 * @class H5P.Interactive360Video
 * @extends H5P.EventDispatcher
 * @property {Object|undefined} editor Set when editing
 * @param {Interactive360VideoParameters} params
 * @param {number} id
 * @param {Object} contentData
 */
function Interactive360Video(params, id, contentData) {
  var self = this;
  var loopVideo;
  ...
  // Create dynamic ids
  self.bookmarksMenuId = 'interactive-360-video-' + this.contentId + '-bookmarks-chooser';
  self.endscreensMenuId = 'interactive-360-video-' + this.contentId + '-endscreens-chooser';
  self.qualityMenuId = 'interactive-360-video-' + this.contentId + '-quality-chooser';
  self.captionsMenuId = 'interactive-360-video-' + this.contentId + '-captions-chooser';
  self.playbackRateMenuId = 'interactive-360-video-' + this.contentId + '-playback-rate-chooser';
  ...
  if (!self.options.video.startScreenOptions.title) {
    self.options.video.startScreenOptions.title = 'Interactive 360 Video';
  }
  ...
  // Three.js 360° video properties
  self.threeScene = null;
  self.threeCamera = null;
  self.threeRenderer = null;
  self.threeSphere = null;
  self.threeVideoTexture = null;
  self.threeCanvas = null;
  self.threeVideoElement = null; // Reference to native video element for aspect ratio
  self.isDragging = false;
  self.previousMousePosition = { x: 0, y: 0 };
  self.cameraRotation = { x: 0, y: Math.PI }; // Initialize to 180 degrees (facing front of sphere)
  self.animationFrameId = null;
  self.raycaster = null; // Three.js raycaster for click detection
  self.mouse = null; // Three.js Vector2 for mouse coordinates
  self.draggingMarker = null; // Currently dragging marker (Editor mode)
  self.dragStartPosition = null; // Starting position when drag begins
  self.cameraControlsEnabled = true; // Flag to enable/disable camera controls
  ...
  this.width = DEFAULT_VIDEO_WIDTH; // parseInt($container.css('width')); // Get width in px
  ...
  self.initialize = function () {
    ...
    self.video.on('stateChange', function (event) {
      ...
      if (self.currentState === Interactive360Video.SEEKING) {
...
// Inheritance
Interactive360Video.prototype = Object.create(H5P.EventDispatcher.prototype);
Interactive360Video.prototype.constructor = Interactive360Video;

/**
 * Set caption tracks for current interactive 360 video
 *
 * @param {H5P.Video.LabelValue[]} tracks
 */
Interactive360Video.prototype.setCaptionTracks = function (tracks) {
  var self = this;
 ...
 */
Interactive360Video.prototype.getCurrentState = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.removeSplash = function () {
...
/**
 * Attach interactive 360 video to DOM element.
 *
 * @param {H5P.jQuery} $container
 */
Interactive360Video.prototype.attach = function ($container) {
  var that = this;
  ...
  $container.addClass('h5p-interactive-360-video').html('');
  ...
  if (!this.video.pressToPlay) {
    if (this.currentState === Interactive360Video.LOADED) {
      // Add all controls
      this.addControls();
    }
    else {
  ...
  this.currentState = Interactive360Video.ATTACHED;
};
 ...
 */
Interactive360Video.prototype.attachVideo = function ($wrapper) {
  var self = this;
  this.video.attach($wrapper);
  
  // Wait for video container to be loaded, then initialize 360° player
  var init360AfterLoad = function() {
    // Use multiple strategies to ensure video element is ready
    setTimeout(function() {
      self.init360Player($wrapper);
    }, 150);
  };
  
  if (this.video && this.video.on) {
    // Listen for container loaded event (video element is in DOM)
    this.video.on('containerLoaded', init360AfterLoad);
    
    // Also listen for loaded event (video metadata ready)
    this.video.on('loaded', function() {
      // Additional delay to ensure everything is ready
      setTimeout(function() {
        self.init360Player($wrapper);
      }, INIT_360_RETRY_DELAY_MS);
    });
    
    // Try immediately if container might already be loaded
    setTimeout(init360AfterLoad, INIT_360_INITIAL_DELAY_MS);
  } else {
    // Fallback: try after a delay
    setTimeout(init360AfterLoad, INIT_360_FALLBACK_DELAY_MS);
  }
...
/**
 * Initialize Three.js 360° video player
 *
 * @param {H5P.jQuery} $wrapper
 */
Interactive360Video.prototype.init360Player = function ($wrapper) {
  var self = this;
  
  // Prevent multiple initializations
  if (self.threeRenderer) {
    return;
  }
  ...
  // Find the native video element - try multiple selectors
  var $videoElement = $wrapper.find('video');
  if ($videoElement.length === 0) {
    // Try finding in the entire video wrapper area
    $videoElement = $wrapper.closest('.h5p-interactive-360-video').find('video');
  }
  
  if ($videoElement.length === 0) {
    // Retry after a delay if video element not found yet
    if (!self._init360RetryCount) {
      self._init360RetryCount = 0;
    }
    if (self._init360RetryCount < INIT_360_MAX_RETRIES) {
      self._init360RetryCount++;
      setTimeout(function() {
        self.init360Player($wrapper);
      }, INIT_360_RETRY_DELAY_MS);
      return;
  ...
  // Store reference to video element for aspect ratio calculation
  self.threeVideoElement = videoElement;
  
  // Ensure video has crossorigin attribute for WebGL texture (critical for CORS)
  if (!videoElement.hasAttribute('crossorigin')) {
    videoElement.setAttribute('crossorigin', 'anonymous');
  }
  
  // Hide the native video element (but keep it in DOM for texture)
  $videoElement.css({
    'position': 'absolute',
    'visibility': 'hidden',
    'opacity': '0',
  ...
  var width = $wrapper.width() || DEFAULT_VIDEO_WIDTH;
  var height = $wrapper.height() || DEFAULT_VIDEO_HEIGHT;
  
  // Create Three.js scene
  self.threeScene = new THREE.Scene();
  
  // Create camera
  self.threeCamera = new THREE.PerspectiveCamera(THREE_CAMERA_FOV, width / height, THREE_CAMERA_NEAR, THREE_CAMERA_FAR);
  self.threeCamera.position.set(0, 0, 0);
  
  // Create sphere geometry (inverted to render inside)
  var geometry = new THREE.SphereGeometry(THREE_SPHERE_RADIUS, THREE_SPHERE_WIDTH_SEGMENTS, THREE_SPHERE_HEIGHT_SEGMENTS);
  geometry.scale(-1, 1, 1); // Invert sphere to render inside
  
  // Create video texture
  self.threeVideoTexture = new THREE.VideoTexture(videoElement);
  self.threeVideoTexture.minFilter = THREE.LinearFilter;
  self.threeVideoTexture.magFilter = THREE.LinearFilter;
  self.threeVideoTexture.format = THREE.RGBFormat;
  
  // Create material with video texture
  var material = new THREE.MeshBasicMaterial({
    map: self.threeVideoTexture
  });
  
  // Create sphere mesh
  self.threeSphere = new THREE.Mesh(geometry, material);
  self.threeScene.add(self.threeSphere);
  
  // Create renderer
  self.threeRenderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
  self.threeRenderer.setSize(width, height);
  self.threeRenderer.setPixelRatio(window.devicePixelRatio);
  
  // Get canvas and set z-index to be behind overlays
  self.threeCanvas = self.threeRenderer.domElement;
  $(self.threeCanvas).css({
    'position': 'absolute',
    'top': '0',
    'left': '0',
    'width': '100%',
  ...
  // Insert canvas into wrapper (before overlay to ensure proper layering)
  $wrapper.prepend(self.threeCanvas);
  
  // Initialize camera rotation (180 degrees = Math.PI to face front of sphere)
  self.cameraRotation = { x: 0, y: Math.PI };
  
  // Initialize raycaster for 3D marker interaction
  self.raycaster = new THREE.Raycaster();
  self.mouse = new THREE.Vector2();
  
  // Setup camera rotation controls
  self.setupCameraControls($wrapper);
  
  // Setup editor drop handler (only in editor mode)
  if (self.editor !== undefined) {
    self.setupEditorDropHandler();
  }
  
  // Start animation loop
  self.animate();
  
  // Update texture when video plays
  if (videoElement.addEventListener) {
    videoElement.addEventListener('play', function() {
      if (self.threeVideoTexture) {
        self.threeVideoTexture.needsUpdate = true;
      }
    });
    ...
    // Trigger resize when video metadata is loaded (to get videoWidth/videoHeight)
  ...
  // Trigger initial resize to set aspect ratio
  setTimeout(function() {
    if (self.$videoWrapper) {
      self.trigger('resize');
    }
...
/**
 * Setup camera rotation controls (mouse/touch drag)
 *
 * @param {H5P.jQuery} $wrapper
 */
Interactive360Video.prototype.setupCameraControls = function ($wrapper) {
  var self = this;
  
  var onMouseDown = function(event) {
    // Don't start camera drag if marker dragging is active or controls disabled
    if (!self.cameraControlsEnabled || self.draggingMarker) {
      return;
    }
    
    // Don't interfere with H5P interactions - check if clicking on overlay/interaction
    var target = event.target;
    var $target = $(target);
    
    // Allow dragging only if not clicking on H5P controls or interactions
    if (!$target.closest('.h5p-overlay, .h5p-interaction, .h5p-controls, .h5p-splash').length) {
      self.isDragging = true;
      self.previousMousePosition = {
        x: event.clientX,
        y: event.clientY
      };
  ...
  var onMouseMove = function(event) {
    // Don't move camera if controls disabled or marker is being dragged
    if (!self.cameraControlsEnabled || self.draggingMarker || !self.isDragging) {
      return;
    }
    
    var deltaX = event.clientX - self.previousMousePosition.x;
    var deltaY = event.clientY - self.previousMousePosition.y;
    
    // Rotate camera based on mouse movement (inverted logic for intuitive control)
    self.cameraRotation.y += deltaX * 0.002; // Yaw (horizontal) - inverted
    self.cameraRotation.x += deltaY * 0.002; // Pitch (vertical) - inverted
    
    // Limit pitch to prevent flipping
    self.cameraRotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, self.cameraRotation.x));
  ...
  var onMouseUp = function(event) {
    // Only stop camera drag if we're not dragging a marker
  ...
  var onTouchMove = function(event) {
    // Don't move camera if controls disabled or marker is being dragged
    if (!self.cameraControlsEnabled || self.draggingMarker || !self.isDragging || event.touches.length !== 1) {
      return;
    }
    
    var touch = event.touches[0];
    var deltaX = touch.clientX - self.previousMousePosition.x;
    var deltaY = touch.clientY - self.previousMousePosition.y;
    
    // Inverted logic for intuitive touch control
    self.cameraRotation.y += deltaX * 0.002;
    self.cameraRotation.x += deltaY * 0.002;
    self.cameraRotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, self.cameraRotation.x));
  ...
  // Attach event listeners to wrapper (not document, to avoid conflicts)
  ...
  self.cleanupCameraControls = function() {
    $wrapper.off('mousedown', onMouseDown);
    $(document).off('mousemove', onMouseMove);
    $(document).off('mouseup', onMouseUp);
    $wrapper.off('touchstart', onTouchStart);
...
/**
 * Resize Three.js 360° player
 *
 * @param {number} width - Width in pixels
 * @param {number} height - Height in pixels
 */
Interactive360Video.prototype.resize360Player = function(width, height) {
  if (!this.threeRenderer || !this.threeCamera || !this.$videoWrapper) {
    return;
  }
 ...
 */
Interactive360Video.prototype.animate = function() {
  var self = this;
  ...
  if (self.threeCamera) {
    var euler = new THREE.Euler(self.cameraRotation.x, self.cameraRotation.y, 0, 'YXZ');
    self.threeCamera.quaternion.setFromEuler(euler);
  }
  ...
  // Update interaction positions (project 3D to 2D)
  self.updateInteractionPositions();
...
/**
 * Helper function to check if an interaction is dynamic in editor mode
 * @param {Object} player - The Interactive360Video instance
 * @param {Object} parameters - Interaction parameters
 * @returns {boolean} True if interaction is dynamic and we're in editor mode
 */
function isDynamicInEditor(player, parameters) {
  return player.editor !== undefined && parameters && parameters.dynamic === true;
}

/**
 * Find the index of an interaction in the interactions array
 * @param {Array} interactions - Array of interaction objects
 * @param {Object} interaction - The interaction to find
 * @returns {number} Index of the interaction, or -1 if not found
 */
function findInteractionIndex(interactions, interaction) {
...
/**
 * Get interaction parameters from options by index
 * @param {Object} options - Player options object
 * @param {number} index - Interaction index
 * @returns {Object|null} Interaction parameters or null if not found
 */
function getInteractionParameters(options, index) {
  return options && options.assets && options.assets.interactions && 
         options.assets.interactions[index] ? 
         options.assets.interactions[index] : null;
}

/**
 * Update interaction positions based on yaw/pitch coordinates
 * Projects 3D spherical coordinates to 2D screen space
 * 
 * IMPORTANT: yaw/pitch represents the TOP-LEFT corner (0,0) of the interaction, not the center.
 * The projection directly gives the top-left screen position - no centering offset is applied.
 */
Interactive360Video.prototype.updateInteractionPositions = function() {
  var self = this;
  
  if (!self.threeScene || !self.threeCamera || !self.interactions) {
    return;
  }
  
  // Loop through all visible interactions
  for (var i = 0; i < self.interactions.length; i++) {
    ...
    var $element = interaction.getElement();
    if (!$element || !$element.length) {
      continue;
    }
    
    // Get parameters
    var parameters = getInteractionParameters(self.options, i);
    
    if (!parameters) {
      continue;
    }
    
    var shouldCheckVisibility = !isDynamicInEditor(self, parameters);
    
    if (shouldCheckVisibility && (!interaction.isVisible || !interaction.isVisible())) {
      $element.css({
        'display': 'none'
      });
      continue;
    ...
    if (self.editor === undefined && parameters.dynamic === true) {
      var hasValidKeyframes = parameters.keyframes && 
                              Array.isArray(parameters.keyframes) && 
                              parameters.keyframes.length > 0;
      if (!hasValidKeyframes) {
        // Dynamic interaction with no keyframes - hide it at runtime
        $element.css({
          'display': 'none'
        });
        continue;
    ...
    // Check if interaction has yaw/pitch coordinates
    var yaw = parameters.yaw;
    var pitch = parameters.pitch;
    
    // If no yaw/pitch, skip (fallback to legacy x/y positioning)
    if (yaw === undefined || pitch === undefined) {
      continue;
    }
    
    if (self.editor === undefined && parameters.dynamic === true && parameters.keyframes && Array.isArray(parameters.keyframes) && parameters.keyframes.length > 0) {
      var time = self.video.getCurrentTime();
      var keyframes = parameters.keyframes;
      var keyframeCount = keyframes.length;
      
      var firstKeyframeTime = Infinity;
      var lastKeyframeTime = -Infinity;
      for (var k = 0; k < keyframeCount; k++) {
        if (keyframes[k].time !== undefined && isFinite(keyframes[k].time)) {
          if (keyframes[k].time < firstKeyframeTime) {
            firstKeyframeTime = keyframes[k].time;
          }
          if (keyframes[k].time > lastKeyframeTime) {
            lastKeyframeTime = keyframes[k].time;
          }
        }
      }
      
      if (time >= firstKeyframeTime && time <= lastKeyframeTime) {
        var prevKeyframe = null;
        var nextKeyframe = null;
        
        for (var k = keyframeCount - 1; k >= 0; k--) {
          if (keyframes[k].time !== undefined && keyframes[k].time <= time) {
        ...
        for (var k = 0; k < keyframeCount; k++) {
          if (keyframes[k].time !== undefined && keyframes[k].time > time) {
            nextKeyframe = keyframes[k];
            break;
          }
        }
        
        if (prevKeyframe === null && nextKeyframe !== null) {
          if (nextKeyframe.yaw !== undefined && isFinite(nextKeyframe.yaw)) {
            yaw = nextKeyframe.yaw;
          }
          if (nextKeyframe.pitch !== undefined && isFinite(nextKeyframe.pitch)) {
            pitch = nextKeyframe.pitch;
          }
        } else if (prevKeyframe !== null && nextKeyframe === null) {
          if (prevKeyframe.yaw !== undefined && isFinite(prevKeyframe.yaw)) {
            yaw = prevKeyframe.yaw;
          }
          if (prevKeyframe.pitch !== undefined && isFinite(prevKeyframe.pitch)) {
            pitch = prevKeyframe.pitch;
          }
        } else if (prevKeyframe !== null && nextKeyframe !== null) {
          var prevTime = prevKeyframe.time;
          var nextTime = nextKeyframe.time;
          var prevYaw = prevKeyframe.yaw !== undefined && isFinite(prevKeyframe.yaw) ? prevKeyframe.yaw : yaw;
          var prevPitch = prevKeyframe.pitch !== undefined && isFinite(prevKeyframe.pitch) ? prevKeyframe.pitch : pitch;
          var nextYaw = nextKeyframe.yaw !== undefined && isFinite(nextKeyframe.yaw) ? nextKeyframe.yaw : yaw;
          var nextPitch = nextKeyframe.pitch !== undefined && isFinite(nextKeyframe.pitch) ? nextKeyframe.pitch : pitch;
          
          var timeDiff = nextTime - prevTime;
          var t = timeDiff > 0 ? (time - prevTime) / timeDiff : 0;
          t = Math.max(0, Math.min(1, t));
          
          pitch = prevPitch + (nextPitch - prevPitch) * t;
          
          var delta = nextYaw - prevYaw;
    ...
    var radius = THREE_SPHERE_RADIUS;
    var y = radius * Math.sin(pitch);
    var x = -radius * Math.cos(pitch) * Math.sin(yaw);
    var z = -radius * Math.cos(pitch) * Math.cos(yaw);
    ...
    // Handle Visibility (Behind Camera?)
    if (vector.z > 1) {
      // Behind camera - hide the element
      $element.css({
        'display': 'none'
      });
      continue;
    } else {
      // In front of camera - FORCE VISIBILITY
      // We must override H5P's default hidden state
      
      // Check if element is being dragged (has h5p-moving class on itself or parent)
      // If dragging, make it transparent (like regular Interactive Video) instead of hiding it
      var isDragging = $element.hasClass('h5p-moving') || 
                       $element.closest('.h5p-dragnbar-element.h5p-moving').length > 0 ||
                       $element.parent().hasClass('h5p-moving');
      
      var opacity = isDragging ? 0.5 : 1; // Semi-transparent when dragging, fully opaque otherwise
      
      $element.css({
        'display': '',
        'visibility': 'visible',
        'opacity': opacity,
        'pointer-events': isDragging ? 'none' : 'auto' // Disable pointer events while dragging
      });
    }
    
    var leftPercent = ((vector.x + 1) / 2) * 100;
    var topPercent = ((-vector.y + 1) / 2) * 100; // Invert Y (screen Y is top-down, CSS is top-down)
    
    // Check if element is being dragged
    // During drag, we update yaw/pitch in real-time via mousemove handler,
    // and this projection (running every frame) makes the interaction follow the mouse cursor
    // The animate loop runs at ~60fps, so updates are smooth and responsive
    var isDragging = $element.hasClass('h5p-moving') || 
                     $element.closest('.h5p-dragnbar-element.h5p-moving').length > 0 ||
                     $element.parent().hasClass('h5p-moving');
    
    // Apply CSS position directly (top-left anchor point)
    // During drag, our real-time yaw/pitch updates + this projection ensure the interaction follows the mouse
    
    // Check if this is a H5P.Link interaction - preserve 'auto' dimensions
    var isLinkInteraction = false;
    if (parameters.action && parameters.action.library) {
      var libraryName = parameters.action.library.split(' ')[0];
      isLinkInteraction = (libraryName === 'H5P.Link');
    }
    ...
    // For link interactions, preserve 'auto' width and height
    // This prevents links from enlarging when browser reflow or other events set explicit dimensions
    if (isLinkInteraction) {
      cssUpdate['width'] = 'auto';
      cssUpdate['height'] = 'auto';
    }
    
    $element.css(cssUpdate);
  }
};

/**
 * Regenerate yaw/pitch coordinates from 2D CSS position
 * Converts the current DOM element position back to 3D spherical coordinates
 * This is called when the editor drag/resize ends to sync the 2D position to 3D space
 * 
 * IMPORTANT: yaw/pitch now represents the TOP-LEFT corner (0,0) of the interaction, not the center.
 * This allows the top-left corner to stay anchored to a specific 3D point (e.g., a person's head)
 * as the camera moves around the 360° scene.
 * 
 * PRECISION: Uses Three.js unproject() method (inverse of project()) to ensure pixel-perfect
 * precision at all screen positions, including edges and corners. This is mathematically
 * consistent with updateInteractionPositions() which uses project().
 * 
 * @param {Object} interaction - H5P interaction object
 * @param {number} [xPercent] - Optional X position in percentage (0-100). If not provided, reads from CSS
 * @param {number} [yPercent] - Optional Y position in percentage (0-100). If not provided, reads from CSS
 */
Interactive360Video.prototype.regenerateYawPitch = function(interaction, xPercent, yPercent) {
  var self = this;
  
  // Need camera for FOV calculation
  if (!self.threeCamera) {
    return;
  }
  ...
  var interactionIndex = findInteractionIndex(self.interactions, interaction);
  ...
  // Get parameters
  var parameters = getInteractionParameters(self.options, interactionIndex);
  
  if (!parameters) {
    return;
  }
  
  var leftPercent = xPercent;
  var topPercent = yPercent;
  var usedProvidedValues = (leftPercent !== undefined && topPercent !== undefined);
  
  if (!usedProvidedValues) {
    var cssLeftStr = $element.css('left') || '';
    var cssTopStr = $element.css('top') || '';
    
    leftPercent = DEFAULT_CENTER_PERCENT;
    topPercent = DEFAULT_CENTER_PERCENT;
    
    if (cssLeftStr.indexOf('%') !== -1) {
      leftPercent = parseFloat(cssLeftStr.replace('%', '')) || DEFAULT_CENTER_PERCENT;
    }
    
    if (cssTopStr.indexOf('%') !== -1) {
      topPercent = parseFloat(cssTopStr.replace('%', '')) || 50;
    }
  }
  ...
  // NDC: x=-1 is left edge, x=+1 is right edge
  //      y=-1 is bottom edge, y=+1 is top edge
  var ndcX = (leftPercent / 100) * NDC_CONVERSION_FACTOR + NDC_MIN;
  var ndcY = -((topPercent / 100) * NDC_CONVERSION_FACTOR + NDC_MIN); // Invert Y (screen Y is top-down, NDC Y is bottom-up)
  
  // Clamp NDC to valid range
  ndcX = Math.max(NDC_MIN, Math.min(NDC_MAX, ndcX));
  ndcY = Math.max(NDC_MIN, Math.min(NDC_MAX, ndcY));
  
  // --- START OF INVERSE PROJECTION LOGIC (Pixel-Perfect Precision) ---
  // Use Three.js unproject() to convert screen position → 3D world space
  // This is the mathematical inverse of project(), ensuring perfect consistency
  // with the rendering path (updateInteractionPositions) which uses project()
  
  // Create NDC vector (z = 0.5 means point at middle of view frustum)
  // We'll normalize and scale to sphere radius after unprojection
  var ndcVector = new THREE.Vector3(ndcX, ndcY, THREE_NDC_Z_COORDINATE);
  
  ndcVector.unproject(self.threeCamera);
  
  // The unprojected point is in world space. Since camera is at origin (0,0,0),
  // the vector from origin to this point is the direction we want.
  // Normalize to get unit direction vector
  ndcVector.normalize();
  
  // Calculate yaw and pitch from normalized direction vector
  // Using same coordinate system as updateInteractionPositions and editor drop handler
  // The coordinate system: x = -cos(pitch)*sin(yaw), y = sin(pitch), z = -cos(pitch)*cos(yaw)
  var pitch = Math.asin(ndcVector.y);  // ndcVector.y is already normalized (-1 to 1)
  var yaw = Math.atan2(-ndcVector.x, -ndcVector.z);  // Match rendering coordinate system
  
  // Normalize Yaw to keep it clean (0 to 2PI)
  yaw = yaw % (2 * Math.PI);
  if (yaw < 0) {
    yaw += 2 * Math.PI;
  }
  
  // --- END OF INVERSE PROJECTION LOGIC ---
  
  // Update interaction data
  parameters.yaw = yaw;
  parameters.pitch = pitch;
  
  // Also store on interaction instance for easy access
  interaction.yaw = yaw;
  interaction.pitch = pitch;
  
  // Trigger move event if available
  if (interaction.trigger) {
    interaction.trigger('move', { yaw: yaw, pitch: pitch });
  }
};

/**
 * Setup editor drop handler to create 3D markers for newly dropped interactions
 * Only active in editor mode
 */
Interactive360Video.prototype.setupEditorDropHandler = function() {
  var self = this;
  ...
  if (!self.threeScene || !self.threeCamera || !self.threeRenderer || !self.threeSphere || !self.raycaster) {
    return;
  }
  
  // Handler for mouseup events (catches drops)
  var onEditorDrop = function(event) {
    // Wait 50ms to let H5P finish its internal drag state
    setTimeout(function() {
      // 1. Identify if we just dropped a NEW interaction (Orphan check)
      var droppedInteraction = null;
      var droppedIndex = -1;
      
      if (self.interactions && self.interactions.length > 0) {
        for (var i = 0; i < self.interactions.length; i++) {
          var interaction = self.interactions[i];
          if (!interaction) {
            continue;
          ...
          var $el = interaction.getElement();
          // If it exists, is NOT dragging, and has NO yaw/pitch -> It was just dropped
          var parameters = getInteractionParameters(self.options, i);
          var hasYawPitch = parameters && (parameters.yaw !== undefined && parameters.pitch !== undefined);
          if ($el && $el.length && !$el.hasClass('h5p-dragging') && !hasYawPitch) {
            droppedInteraction = interaction;
            droppedIndex = i;
            break; // Found the dropped interaction
          }
        }
      }
      
      if (!droppedInteraction || droppedIndex === -1) {
        return; // No dropped interaction found
      }
      
      // 2. Perform Raycast from Mouse Position
      // Calculate mouse coordinates relative to the canvas/renderer
      var rect = self.threeRenderer.domElement.getBoundingClientRect();
      var mouse = new THREE.Vector2();
      mouse.x = ((event.clientX - rect.left) / rect.width) * 2 - 1;
      mouse.y = -((event.clientY - rect.top) / rect.height) * 2 + 1;
      
      // Set up raycaster
      self.raycaster.setFromCamera(mouse, self.threeCamera);
      var intersects = self.raycaster.intersectObject(self.threeSphere, false);
      
      if (intersects.length > 0) {
        // 3. Get precise 3D point
        var hitPoint = intersects[0].point.clone();
        
        // The sphere is inverted (scale -1, 1, 1), so we need to adjust
        // The intersection point is on the inside surface, so we need to flip it
        hitPoint.negate();
        
        // Normalize to unit vector
        var point = hitPoint.normalize();
        
        // 4. Calculate Yaw and Pitch from intersection point
        var pitch = Math.asin(point.y);
        var yaw = Math.atan2(point.x, -point.z);
        
        // 5. Save Yaw/Pitch to H5P Data
        // Get parameters
        var parameters = getInteractionParameters(self.options, droppedIndex);
        
        if (parameters) {
          // Save yaw and pitch to parameters
          parameters.yaw = yaw;
          parameters.pitch = pitch;
          
          // Also store on interaction instance for easy access
          droppedInteraction.yaw = yaw;
          droppedInteraction.pitch = pitch;
          
          // Trigger update to immediately position the element
          // The animate loop will handle the CSS positioning via updateInteractionPositions()
          if (droppedInteraction.trigger) {
            droppedInteraction.trigger('move');
          }
        }
      }
    }, 50); // Small delay to allow H5P to finish drag state
  };
  
  // Add event listener
  document.addEventListener('mouseup', onEditorDrop);
  
  // Store cleanup function
  self.cleanupEditorDropHandler = function() {
    document.removeEventListener('mouseup', onEditorDrop);
  };
};
 ...
 */
Interactive360Video.prototype.cleanup360Player = function() {
  var self = this;
  ...
  if (self.cleanupCameraControls) {
    self.cleanupCameraControls();
  }
  
  if (self.cleanupEditorDropHandler) {
    self.cleanupEditorDropHandler();
  }
  
  if (self.threeVideoTexture) {
    self.threeVideoTexture.dispose();
    self.threeVideoTexture = null;
  }
  
  if (self.threeRenderer) {
    self.threeRenderer.dispose();
    if (self.threeCanvas && self.threeCanvas.parentNode) {
      self.threeCanvas.parentNode.removeChild(self.threeCanvas);
    }
    self.threeRenderer = null;
    self.threeCanvas = null;
  }
  ...
  if (self.threeSphere) {
    if (self.threeSphere.geometry) self.threeSphere.geometry.dispose();
    if (self.threeSphere.material) self.threeSphere.material.dispose();
    self.threeSphere = null;
  }
 ...
 * Add splash screen
 */
Interactive360Video.prototype.addSplash = function () {
  var that = this;
 ...
 */
Interactive360Video.prototype.getDuration = function () {
...
/**
 * Update and show controls for the interactive 360 video.
 */
Interactive360Video.prototype.addControls = function () {
  const self = this;
  ...
  const humanTime = Interactive360Video.humanizeTime(duration);
  const a11yTime = Interactive360Video.formatTimeForA11y(duration, self.l10n);
    ...
    for (let i = this.options.assets.endscreens.length-1; i>=0; i--) {
      ...
      if (endscreen.time > endTime) {
        if (!haveEndscreenMovedToEnd) {
          ...
          this.options.assets.endscreens[i].label = Interactive360Video.humanizeTime(endTime) + ' ' + this.l10n.endscreen;
 ...
 */
Interactive360Video.prototype.loaded = function () {
  // Get duration
  ...
  if (this.currentState === Interactive360Video.ATTACHED) {
    if (!this.video.pressToPlay) {
      this.addControls();
    }
  ...
  this.currentState = Interactive360Video.LOADED;
};
 ...
 *
 * @param {number} index
 * @returns {H5P.Interactive360VideoInteraction}
 */
Interactive360Video.prototype.initInteraction = function (index) {
  var self = this;
  var parameters = self.options.assets.interactions[index];

  // Optimization: Sort keyframes by time for dynamic interactions
  if (parameters.dynamic === true && parameters.keyframes && Array.isArray(parameters.keyframes)) {
    parameters.keyframes.sort(function(a, b) {
  ...
  interaction.on('display', function (event) {
    ...
    // Hide visual DOM element but keep it for H5P functionality
    if (self.threeScene && self.threeCamera) {
      $interaction.css({
        'visibility': 'hidden',
        'pointer-events': 'none'
      });
      
      // Interactions are positioned via updateInteractionPositions() in animate loop
      // No need to create 3D markers - using yaw/pitch based positioning
    }
  ...
  interaction.on('hide', function () {
    // Interactions are positioned via updateInteractionPositions() in animate loop
    // No need to clear 3D markers - using yaw/pitch based positioning
    self.handleAnswered();
  });
 ...
 */
Interactive360Video.prototype.handleAnswered = function () {
  const self = this;
 ...
 */
Interactive360Video.prototype.getAnsweredTotal = function () {
  return this.interactions
    .filter(a => a.getProgress() === Interaction.PROGRESS_ANSWERED).length;
};

/**
 * Checks if the interactive 360 video should have summary task scheduled at
 * the end of the video.
 *
 * This is the summary created in the summary tab of the editor.
 *
 * @returns {boolean}
 *   true if this interactive 360 video has a summary
 *   false otherwise
 */
Interactive360Video.prototype.hasMainSummary = function () {
 ...
 */
Interactive360Video.prototype.addSliderInteractions = function () {
  const self = this;
  
  if (!this.controls || !this.controls.$interactionsContainer) {
    return;
  }
  ...
  if (this.interactionKeyboardControls) {
    this.interactionKeyboardControls.elements = [];
  }

  // Add new dots
  // In editor mode: only show white dots for static interactions
  // In runtime mode: show white dots for all interactions (static and dynamic)
  //   - Static interactions: white dot at their start time
  //   - Dynamic interactions: white dot at their first keyframe time
  H5P.jQuery.extend([], this.interactions)
    .sort((a, b) => a.getDuration().from - b.getDuration().from)
    .forEach(interaction => {
      // Get interaction parameters to check if it's dynamic
      var interactionIndex = findInteractionIndex(self.interactions, interaction);
      
      var parameters = getInteractionParameters(self.options, interactionIndex);
      
      // In editor mode, skip dynamic interactions (they get keyframe markers instead)
      if (self.editor !== undefined && parameters && parameters.dynamic === true) {
        return; // Skip this interaction in editor
      }
      
      // In runtime mode, dynamic interactions are allowed - they'll use first keyframe time via getDuration()
  ...
  // In editor mode, also add keyframe markers for dynamic interactions
  if (self.editor !== undefined) {
    self.addKeyframeMarkers();
  }
};

/**
 * Add keyframe markers for dynamic interactions (editor mode only).
 * Collects all keyframes from all dynamic interactions, deduplicates by time,
 * and creates orange markers below the timeline.
 */
Interactive360Video.prototype.addKeyframeMarkers = function () {
  const self = this;
  ...
  // Create keyframes container if it doesn't exist (might not be created yet)
  if (!self.controls.$keyframesContainer) {
    self.controls.$keyframesContainer = $('<div/>', {
      'role': 'menu',
      'class': 'h5p-keyframes-container',
      'aria-label': 'Keyframes',
    });
    // Append to slider container if it exists
    if (self.$sliderContainer) {
      self.controls.$keyframesContainer.appendTo(self.$sliderContainer);
    }
  }
  
  // Remove old keyframe markers
  self.controls.$keyframesContainer.children().remove();
  
  // Collect all keyframe times from all dynamic interactions
  var keyframeTimes = new Set(); // Use Set to automatically deduplicate
  
  // In editor mode, get parameters from editor.params directly to ensure we have the latest data
  var paramsSource = null;
  if (self.editor && self.editor.params && self.editor.params.interactions) {
    paramsSource = self.editor.params.interactions;
  } else if (self.options && self.options.assets && self.options.assets.interactions) {
    paramsSource = self.options.assets.interactions;
  }
  
  if (!paramsSource) {
    return; // Can't get parameters
  }
  ...
  for (var i = 0; i < self.interactions.length; i++) {
    ...
    // Get interaction parameters from the source (editor params or options)
    var parameters = paramsSource[i] || null;
    
    // Only process dynamic interactions
    if (!parameters || parameters.dynamic !== true) {
      continue;
    }
    
    // Collect keyframe times from this interaction
    if (parameters.keyframes && Array.isArray(parameters.keyframes) && parameters.keyframes.length > 0) {
      parameters.keyframes.forEach(function(keyframe) {
        if (keyframe.time !== undefined && keyframe.time !== null && isFinite(keyframe.time)) {
  ...
  var sortedKeyframeTimes = Array.from(keyframeTimes).sort(function(a, b) {
    return a - b;
  });
  
  // Ensure oneSecondInPercentage is initialized (calculate if needed)
  if (self.oneSecondInPercentage === undefined || self.oneSecondInPercentage === null) {
    var duration = self.getDuration();
    if (duration > 0) {
      self.oneSecondInPercentage = 100 / duration;
    } else {
      return; // Can't calculate position without duration
    }
  }
  
  // Create markers for each unique keyframe time
  sortedKeyframeTimes.forEach(function(keyframeTime) {
    var timeStr = H5P.Interactive360Video ? H5P.Interactive360Video.humanizeTime(keyframeTime) : keyframeTime.toFixed(2) + 's';
    var $marker = $('<div/>', {
      'role': 'menuitem',
      'class': 'h5p-seekbar-keyframe',
      'aria-label': 'Keyframe. ' + timeStr,
      'title': 'Keyframe at ' + timeStr,
      'css': {
        left: (keyframeTime * self.oneSecondInPercentage) + '%'
      },
      'on': {
        click: function(e) {
          e.preventDefault();
          e.stopPropagation();
          self.seekToKeyframe(keyframeTime);
        },
    ...
    // Handle prevent skipping (same as white dots)
    ...
    if (self.controls.$keyframesContainer) {
      $marker.appendTo(self.controls.$keyframesContainer);
      
      // Add to keyboard controls (same as white dots) - keyboard controls manage tabindex
      if (self.preventSkippingMode !== 'both' && self.interactionKeyboardControls) {
        self.interactionKeyboardControls.addElement($marker.get(0));
      }
    }
  });
};
...
/**
 * Seek to a specific keyframe time.
 * Similar to selectDot() but for keyframe markers.
 * @param {number} keyframeTime The keyframe time to seek to
 */
Interactive360Video.prototype.seekToKeyframe = function (keyframeTime) {
  const self = this;
  
  if (self.isSkippingProhibited(keyframeTime)) {
    self.showPreventSkippingMessage(
      {
        x: keyframeTime / self.video.getDuration() * self.controls.$slider.get(0).offsetWidth,
        y: 13 // Below timeline
      },
  ...
  // Skip if already on given timecode (same logic as selectDot)
  if (Math.floor(self.video.getCurrentTime() * 10) === Math.floor(keyframeTime * 10)) {
    return;
  }
  
  if (self.currentState === H5P.Video.VIDEO_CUED) {
    self.play();
    self.seek(keyframeTime);
  }
  else if (self.currentState === H5P.Video.PLAYING) {
    self.seek(keyframeTime);
  }
  else {
    self.play(); // for updating the slider
    self.seek(keyframeTime);
    self.pause();
  }
};
 ...
 */
Interactive360Video.prototype.handleInteractionTabIndex = function (event) {
 ...
 */
Interactive360Video.prototype.closePopupMenus = function ($exceptButton) {
 ...
 * @return {boolean}
 */
Interactive360Video.prototype.displayBookmarks = function () {
 ...
 */
Interactive360Video.prototype.addBookmarks = function () {
  this.bookmarksMap = {};
 ...
 * Puts all the cool narrow lines around the slider / seek bar.
 */
Interactive360Video.prototype.addEndscreenMarkers = function () {
  this.endscreensMap = {};
 ...
 */
Interactive360Video.prototype.addBubbles = function () {
  if (!this.editor && this.hasStar) {
    ...
    this.bubbleEndscreen = new Bubble(
      this.$star,
      {
        ...
        maxWidth: 'auto',
        style: 'h5p-interactive-360-video-bubble-endscreen',
        mode: 'full'
      }
    );
  }
 ...
 */
Interactive360Video.prototype.toggleBookmarksChooser = function (show, params = {initialLoad: false, keepStopped: false, firstPlay: false}) {
 ...
 * @param {boolean} [params.firstPlay] If first time.
 */
Interactive360Video.prototype.toggleEndscreensChooser = function (show, params = {keepStopped: false, firstPlay: false}) {
 ...
 */
Interactive360Video.prototype.updateChooserTime = function ($chooser, selector) {
  const $addElement = $chooser.find(selector);
  $addElement.html($addElement.data('default').replace('@timecode', Interactive360Video.humanizeTime(this.video.getCurrentTime())));
};

/**
 * Will interrupt the video and remember that is was interrupted for resuming later.
 */
Interactive360Video.prototype.interruptVideo = function () {
 ...
 */
Interactive360Video.prototype.resumeVideo = function (override) {
  if (!override) {
 ...
 */
Interactive360Video.prototype.toggleEndscreen = function (show) {
  if (
  ...
  if (show) {
    this.disableTabIndexes('.h5p-interactive-360-video-endscreen');
 ...
 */
Interactive360Video.prototype.showPreventSkippingMessage = function (offset = {}, message) {
  var self = this;
  ...
  self.preventSkippingWarningTimeout = setTimeout(function () {
    ...
    }, PREVENT_SKIPPING_WARNING_DELAY_MS);
  }, 2000);
};
 ...
 */
Interactive360Video.prototype.onBookmarkSelect = function ($bookmark, bookmark) {
  const self = this;
  ...
  const l11yTime = Interactive360Video.formatTimeForA11y(bookmark.time, self.l10n);
 ...
 */
Interactive360Video.prototype.onEndscreenSelect = function ($endscreenMarker, endscreen) {
  const self = this;
  ...
  const l11yTime = Interactive360Video.formatTimeForA11y(endscreen.time, self.l10n);
  setTimeout(() => self.read(`${self.l10n.currentTime} ${l11yTime}`), 150);
};
 ...
 * @returns {H5P.jQuery}
 */
Interactive360Video.prototype.addBookmark = function (id, tenth) {
  var self = this;
 ...
 * @returns {H5P.jQuery}
 */
Interactive360Video.prototype.addEndscreen = function (id, tenth) {
  var self = this;
 ...
 * @param {H5P.jQuery} $wrapper
 */
Interactive360Video.prototype.attachControls = function ($wrapper) {
  var self = this;
  ...
  const textFullTime = Interactive360Video.formatTimeForA11y(0, self.l10n);
  ...
  // Add container for keyframe markers (below timeline, editor only)
  self.controls.$keyframesContainer = $('<div/>', {
    'role': 'menu',
    'class': 'h5p-keyframes-container',
    'aria-label': 'Keyframes',
  ...
  self.controls.$slider = $('<div/>', {appendTo: self.$sliderContainer}).slider({
    ...
    create: function (event) {
      ...
      $handle
        ...
        .attr('aria-valuetext', Interactive360Video.formatTimeForA11y(0, self.l10n))
    ...
    start: function () {
      ...
      self.currentState = Interactive360Video.SEEKING;
  ...
  // Append keyframes container below timeline (editor only)
  if (self.controls.$keyframesContainer) {
    self.controls.$keyframesContainer.appendTo(self.$sliderContainer);
  }
 ...
 */
Interactive360Video.prototype.playStarAnimation = function () {
  const self = this;
 ...
 */
Interactive360Video.prototype.playBubbleAnimation = function (text) {
 ...
 */
Interactive360Video.prototype.hasActivePauseInteraction = function () {
 ...
 */
Interactive360Video.prototype.createButton = function (type, extraClass, $target, handler, text) {
  var self = this;
  var options = {
    role: 'button',
    tabindex: 0,
 ...
 */
Interactive360Video.prototype.addQualityChooser = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.updateQuality = function (quality) {
  var self = this;
 ...
 */
Interactive360Video.prototype.addPlaybackRateChooser = function () {
  var self = this;
...
Interactive360Video.prototype.updatePlaybackRate = function (rate) {
  var self = this;
 ...
 */
Interactive360Video.prototype.startUpdatingBufferBar = function () {
  var self = this;
  if (self.bufferLoop) {
    return;
  }
  ...
  var updateBufferBar = function () {
    ...
    self.bufferLoop = setTimeout(updateBufferBar, BUFFER_UPDATE_INTERVAL_MS);
  };
  updateBufferBar();
};
 ...
 */
Interactive360Video.prototype.resize = function () {
  // Not yet attached
  if (!this.$container) {
    return;
  }
  ...
  // Calculate aspect ratio for 360° video
  var aspectRatio = 16 / 9; // Default to 16:9 (1.777...)
  
  // If Three.js player is initialized and video is loaded, use actual video aspect ratio
  if (this.threeVideoElement && this.threeVideoElement.videoWidth && this.threeVideoElement.videoHeight) {
    aspectRatio = this.threeVideoElement.videoWidth / this.threeVideoElement.videoHeight;
  }
  ...
  if (fullscreenOn) {
    // In fullscreen, respect container height constraints
    if (calculatedHeight + controlsHeight <= containerHeight) {
      // Video fits with calculated height
      videoHeight = calculatedHeight;
      width = this.$container.width();
      this.$videoWrapper.css({
        width: width + 'px',
        height: videoHeight + 'px',
        marginTop: (containerHeight - controlsHeight - videoHeight) / 2 + 'px'
      });
      ...
      // Video is too tall, scale to fit container
      videoHeight = containerHeight - controlsHeight;
      width = videoHeight * aspectRatio;
      this.$videoWrapper.css({
        marginLeft: (this.$container.width() - width) / 2 + 'px',
        width: width + 'px',
        height: videoHeight + 'px'
      });
    ...
    // Normal (non-fullscreen) mode: use calculated height based on width
    width = this.$container.width();
    videoHeight = calculatedHeight;
  ...
  this.resize360Player(width, videoHeight);
...
/**
 * Determine if interactive 360 video should be in mobile view.
 */
Interactive360Video.prototype.resizeMobileView = function () {
  // IV not init
 ...
 */
Interactive360Video.prototype.resizeInteractions = function () {
  // IV not init
  if (isNaN(this.currentState)) {
    return;
  }
 ...
 */
Interactive360Video.prototype.recreateCurrentInteractions = function () {
 ...
 */
Interactive360Video.prototype.resizeStartScreen = function () {
 ...
 */
Interactive360Video.prototype.toggleFullScreen = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.timeUpdate = function (time, skipNextTimeUpdate) {
  var self = this;
 ...
 */
Interactive360Video.prototype.setSliderPosition = function (time) {
  try {
    const sliderHandle = this.controls.$slider.find('.ui-slider-handle');
    const timePassedText = Interactive360Video.formatTimeForA11y(time, this.l10n);
 ...
 * @param {number} time
 */
Interactive360Video.prototype.updateInteractions = function (time) {
  var self = this;
  ...
  if (tenth !== self.lastTenth) {
    ...
    const regularEndscreenHit = function () {
      return self.endscreensMap !== undefined && self.endscreensMap[tenth] !== undefined && self.currentState !== Interactive360Video.SEEKING;
    };
 ...
 */
Interactive360Video.prototype.updateCurrentTime = function (seconds) {
  ...
  const humanTime = Interactive360Video.humanizeTime(seconds);
  const a11yTime = Interactive360Video.formatTimeForA11y(seconds, self.l10n);
 ...
 * @returns {number}
 */
Interactive360Video.prototype.getUsersScore = function () {
  var score = 0;
 ...
 * @returns {number}
 */
Interactive360Video.prototype.getUsersMaxScore = function () {
  var maxScore = 0;
 ...
 * @returns {number}
 */
Interactive360Video.prototype.getScore = function () {
 ...
 * @returns {number}
 */
Interactive360Video.prototype.getMaxScore = function () {
 ...
 */
Interactive360Video.prototype.showOverlayMask = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.restorePosterTabIndexes = function () {
  const self = this;
 ...
 */
Interactive360Video.prototype.disableTabIndexes = function (elementToExclude = '.h5p-dialog-wrapper') {
  var self = this;
 ...
 */
Interactive360Video.prototype.restoreTabIndexes = function ($withinContainer) {
  var self = this;
 ...
 * within them.
 */
Interactive360Video.prototype.toggleFocusTrap = function () {
 ...
 *
 * @param {H5P.Interactive360VideoInteraction[]} requiredInteractions
 * @param {jQuery} $focusedElement
 */
Interactive360Video.prototype.trapFocusInInteractions = function (requiredInteractions, $focusedElement) {
 ...
 * @return {jQuery} the dialog wrapper element
 */
Interactive360Video.prototype.hideOverlayMask = function () {
  var self = this;
 ...
 * @returns {jQuery}
 */
Interactive360Video.prototype.showWarningMask = function () {
  const self = this;
  const warningTextId = `interactive-360-video-${self.contentId}-${self.instanceIndex}-completion-warning-text`;
 ...
 * @return {jQuery}
 */
Interactive360Video.prototype.setDisabled = $element => {
  return $element
    .attr('aria-disabled', 'true')
    .attr('tabindex', '-1');
};
 ...
 * @return {boolean}
 */
Interactive360Video.prototype.isDisabled = $element => {
 ...
 * @return {jQuery}
 */
Interactive360Video.prototype.removeDisabled = $element => {
  return $element
 ...
 */
Interactive360Video.prototype.hasUncompletedRequiredInteractions = function (second) {
  var self = this;
 ...
 *
 * @return {H5P.Interactive360VideoInteraction[]} visible interactions
 */
Interactive360Video.prototype.getVisibleInteractions = function () {
 ...
 * @return {H5P.Interactive360VideoInteraction[]} visible interactions
 */
Interactive360Video.prototype.getVisibleInteractionsAt = function (second) {
  return this.interactions.filter(function (interaction) {
 ...
 */
Interactive360Video.prototype.setUserSubmitted = function (state) {
 ...
 */
Interactive360Video.prototype.showSolutions = function () {
 ...
 */
Interactive360Video.prototype.getAnswerGiven = function () {
  /*
 ...
 * @returns {string}
 */
Interactive360Video.prototype.getTitle = function () {
  return H5P.createTitle((this.contentData && this.contentData.metadata && this.contentData.metadata.title) ? this.contentData.metadata.title : 'Interactive 360 Video');
};
 ...
 */
Interactive360Video.prototype.findNextInteractionToShow = function (time, index) {
  let candidate;
  for (var i = 0; i < this.interactions.length; i++) {
    // Skip if this interaction is already visible
    if (this.interactions[i].isVisible()) {
      continue;
    }
    
    // Skip if this is the interaction we just processed (to avoid infinite loops)
    ...
    // Get interaction parameters to check if it's dynamic
    var parameters = this.options && this.options.assets && this.options.assets.interactions && 
                    this.options.assets.interactions[i] ? this.options.assets.interactions[i] : null;
    
    // Check if this is a dynamic interaction (both editor and runtime)
    var isDynamic = parameters && parameters.dynamic === true;
    
    // Check if this interaction should be visible at the current time
    var shouldBeVisible = this.interactions[i].visibleAt(time);
    
    if (shouldBeVisible) {
      // This interaction should be visible but isn't - it's a candidate
      // For static interactions, break immediately to show them in order
      if (!isDynamic) {
        candidate = i;
        break;
      }
      // For dynamic interactions, find the one with the earliest start time that we haven't shown yet
      // This ensures we show all dynamic interactions that should be visible
      if (candidate === undefined || duration.from < this.interactions[candidate].getDuration().from) {
        candidate = i;
      }
    }
    else if (!shouldBeVisible && (duration.from > time || (duration.from == time && (index === undefined || i > index)))
        && (candidate === undefined || duration.from < this.interactions[candidate].getDuration().from)) {
      // Future interaction (not yet visible) - only consider if no visible candidate yet
      candidate = i;
    }
  }
  return candidate;
 ...
 */
Interactive360Video.prototype.findNextInteractionToHide = function (time) {
  let candidate;
 ...
 * @param {number} time
 */
Interactive360Video.prototype.hideInteractions = function (time) {
  // Loop through all visible interactions and hide any that shouldn't be visible at this time
  // This works for both forward playback and backward seeking
  // Iterate backwards to safely remove items from array while looping
  for (let i = this.visibleInteractions.length - 1; i >= 0; i--) {
    const interactionIndex = this.visibleInteractions[i];
    const interaction = this.interactions[interactionIndex];
    
    if (interaction && !interaction.visibleAt(time)) {
    // Hide this interaction
    interaction.toggle(time);

      // Remove from visibleInteractions array
      this.visibleInteractions.splice(i, 1);
    }
  }

  // Reset nextInteractionToHide since we've processed all interactions
 ...
 * @param {number} time
 */
Interactive360Video.prototype.showInteractions = function (time) {
  ...
  while (interaction) {
    // Get interaction parameters to check if it's dynamic
    var interactionIndex = this.nextInteractionToShow;
    var parameters = this.options && this.options.assets && this.options.assets.interactions && 
                    this.options.assets.interactions[interactionIndex] ? this.options.assets.interactions[interactionIndex] : null;
    
    // Check if this is a dynamic interaction (both editor and runtime)
    var isDynamic = parameters && parameters.dynamic === true;
    
    // For dynamic interactions (editor or runtime), use visibleAt() which handles them correctly
    // For static interactions, check duration.from
    var shouldShow = false;
    if (isDynamic) {
      // Dynamic interaction - use visibleAt() which returns true for full video duration
      shouldShow = interaction.visibleAt(time);
    } else {
      // Normal behavior for static interactions - check duration.from
      shouldShow = interaction.getDuration().from <= time;
    }
    
    if (!shouldShow) {
      break; // No more interactions to show
    }
 ...
 * @param {number} time
 */
Interactive360Video.prototype.toggleInteractions = function (time) {
...
/**
 * Start interactive 360 video playback.
 */
Interactive360Video.prototype.play = function () {
  this.video.play();
};

/**
 * Seek interactive 360 video to the given time
 * @param {number} [time = 0] Time in seconds to seek to.
 * @param {object} [options = {}] Options.
 * @param {boolean} [options.force] If true, ignore skipping prohibited.
 */
Interactive360Video.prototype.seek = function (time = 0, options = {}) {
...
/**
 * Pause interactive 360 video playback.
 */
Interactive360Video.prototype.pause = function () {
 ...
 */
Interactive360Video.prototype.resetTask = function () {
 ...
 */
Interactive360Video.prototype.read = function (content) {
  const self = this;
 ...
 */
Interactive360Video.prototype.getCopyrights = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.isSkippingProhibited = function (time = 0) {
  if (this.editor) {
    return false;
  }
...
/** @constant {number} */
Interactive360Video.SEEKING = 4;
/** @constant {number} */
Interactive360Video.LOADED = 5;
/** @constant {number} */
Interactive360Video.ATTACHED = 6;
 ...
 * @returns {string}
 */
Interactive360Video.humanizeTime = function (seconds) {
  const time = Interactive360Video.secondsToMinutesAndHours(seconds);
  let result = '';
 ...
 * @return {string}
 */
Interactive360Video.formatTimeForA11y = function (seconds, labels) {
  const time = Interactive360Video.secondsToMinutesAndHours(seconds);
 ...
 * @return {Time}
 */
Interactive360Video.secondsToMinutesAndHours = function (seconds) {
 ...
 */
Interactive360Video.prototype.getXAPIData = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.getContext = function () {
  var self = this;
 ...
 */
Interactive360Video.prototype.showInteractionsAssistance = function () {
  const self = this;
...
export default Interactive360Video;
