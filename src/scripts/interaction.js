/**
 * H5P Interactive 360 Video - Interaction Module
 * 
 ...
 * @property {boolean} mainSummary True if this interaction is the built-in summary of Interactive 360 Video.
...
/**
 * Keeps control of interactions in the interactive 360 video.
 *
 * @class H5P.Interactive360VideoInteraction
 * @extends H5P.EventDispatcher
 * @param {Parameters} parameters describes action behavior
 * @param {H5P.Interactive360Video} player instance
...
function Interaction(parameters, player, previousState) {
  ...
  var createButton = function (preventAnimation) {
    ...
    if (self.getRequiresCompletion() &&
        ...
        player.currentState !== H5P.Interactive360Video.SEEKING) {
      openDialog(true);
    }
   ...
   *
   * @return {Array.<H5P.Interactive360VideoInteraction>}
  ...
  self.getDuration = function () {
    if (parameters.dynamic === true && player && player.video) {
      var videoDuration = player.video.getDuration();
      
      if (player.editor !== undefined) {
        return {
          from: 0,
          to: videoDuration + 1 // Make sure that all interactions display at least one second to be consistent with the old behaviour
        };
      }
      
      var firstKeyframeTime = 0;
      var lastKeyframeTime = videoDuration;
      
      if (parameters.keyframes && Array.isArray(parameters.keyframes) && parameters.keyframes.length > 0) {
        var keyframeTimes = parameters.keyframes
          .map(function(kf) { return kf.time !== undefined ? kf.time : null; })
          .filter(function(t) { return t !== null && isFinite(t); });
        
        if (keyframeTimes.length > 0) {
          firstKeyframeTime = Math.min.apply(null, keyframeTimes);
          lastKeyframeTime = Math.max.apply(null, keyframeTimes);
        }
      }
      
      return {
        from: firstKeyframeTime,
        to: lastKeyframeTime + 1 // Make sure that all interactions display at least one second to be consistent with the old behaviour
      };
    }
  ...
  self.selectDot = function () {
    // Use getDuration() to get the correct start time (for dynamic interactions at runtime, this is the first keyframe time)
    var duration = self.getDuration();
    var startTime = duration.from;
    
    if (player.isSkippingProhibited(startTime)) {
      player.showPreventSkippingMessage(
        {
          x: startTime / player.video.getDuration() *
     ...
     * in close succession.
     */
    if (Math.floor(player.video.getCurrentTime() * 10) === Math.floor(startTime * 10)) {
      return;
    }
  ...
  self.addDot = function () {
    ...
    // One could also set width using ((duration.to - duration.from + 1) * player.oneSecondInPercentage)
    const $menuitem = $('<div/>', {
      'role': 'menuitem',
      ...
      title: title,
      css: {
        left: (duration.from * player.oneSecondInPercentage) + '%'
      },
      on: {
  ...
  self.visibleAt = function (time) {
    // If interaction is dynamic and we're in the editor, always show it
    // This allows users to scrub the video and place keyframes at any time
    if (parameters.dynamic && player.editor !== undefined) {
      return true;
    }
    
    // For dynamic interactions at runtime, use getDuration() which returns first keyframe to last keyframe
    // This ensures they only appear between first and last keyframe
    if (parameters.dynamic === true) {
      var duration = self.getDuration();
      return !(time < duration.from || time >= duration.to);
    }
    
    // For static interactions, use original duration values
  ...
  self.getCopyrights = function () {
    if (!self.isStandaloneLabel()) {
      ...
      if (instance !== undefined) {
        ...
        interactionCopyrights.setLabel(title + ' ' + H5P.Interactive360Video.humanizeTime(parameters.duration.from) + ' - ' + H5P.Interactive360Video.humanizeTime(parameters.duration.to));
  ...
  self.getClipboardData = function () {
    return H5P.DragNBar.clipboardify(H5PEditor.Interactive360Video.clipboardKey, parameters, 'action');
  };
